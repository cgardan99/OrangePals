...
INSERT INTO PAIS (NOMBRE) VALUES ("Côte d'Ivoire");
...
CREATE FUNCTION COUNT_CM(P_ID INT) RETURNS INT
BEGIN
    DECLARE Y INT DEFAULT 0;
    SET Y = (SELECT COUNT(*) FROM COMENTARIO WHERE PUBLICACION_ID = P_ID);
    RETURN Y;
END __

CREATE FUNCTION COUNT_COR(P_ID INT) RETURNS INT
BEGIN
    DECLARE Y INT DEFAULT 0;
    SET Y = (SELECT COUNT(*) FROM CORAZON WHERE PUBLICACION_ID = P_ID);
    RETURN Y;
END __

CREATE FUNCTION COUNT_BMARK(P_ID INT, USUARIO_ID INT) RETURNS INT
BEGIN
    DECLARE Y INT DEFAULT 0;
    SET Y = (SELECT COUNT(*) FROM BMARK WHERE P_ID = PUBLICACION_ID AND USUARIO_ID = USUARIO_ID);
    RETURN Y;
END __

CREATE PROCEDURE ARMAR_LISTA_PUBLICACIONES(IN USUARIO_ID INT)
BEGIN
    SELECT P.ID, P.TITULO, SUBSTRING(P.TEXTO_PUBLICACION, 1, 200), COUNT_COR(P.ID), COUNT_CM(P.ID), COUNT_BMARK(P.ID, USUARIO_ID), ES_PUBLICACION_MIA(P.ID, USUARIO_ID), P.F_PUBLICACION FROM PUBLICACION P;
END __

CREATE PROCEDURE ARMAR_LISTA_PUBLICACIONES_BMARKS(IN U_ID INT)
BEGIN
    SELECT P.ID, P.TITULO, SUBSTRING(P.TEXTO_PUBLICACION, 1, 200), COUNT_COR(P.ID), COUNT_CM(P.ID), COUNT_BMARK(P.ID, U_ID), ES_PUBLICACION_MIA(P.ID, U_ID), P.F_PUBLICACION FROM PUBLICACION P WHERE P.ID IN (SELECT DISTINCT PUBLICACION_ID FROM BMARK WHERE USUARIO_ID = U_ID);
END __

CREATE PROCEDURE ARMAR_LISTA_PUBLICACIONES_MIA(IN U_ID INT)
BEGIN
    SELECT P.ID, P.TITULO, SUBSTRING(P.TEXTO_PUBLICACION, 1, 200), COUNT_COR(P.ID), COUNT_CM(P.ID), COUNT_BMARK(P.ID, U_ID), ES_PUBLICACION_MIA(P.ID, U_ID), P.F_PUBLICACION FROM PUBLICACION P WHERE P.USUARIO_ID = U_ID;
END __

CREATE FUNCTION ES_PUBLICACION_MIA(PUBLICACION_ID INT, U_ID INT) RETURNS INT
BEGIN
    DECLARE MIA INT DEFAULT 0;
    SET MIA = (SELECT COUNT(*) FROM PUBLICACION WHERE ID = PUBLICACION_ID AND USUARIO_ID = U_ID);
    RETURN MIA;
END __

CREATE FUNCTION LA_GUARDE_YO(P_ID INT, U_ID INT) RETURNS INT
BEGIN
    DECLARE GUARDADO INT DEFAULT 0;
    SET GUARDADO = (SELECT COUNT(*) FROM BMARK WHERE PUBLICACION_ID = P_ID AND USUARIO_ID = U_ID);
    RETURN GUARDADO;
END __

CREATE FUNCTION ES_COMENTARIO_MIO(COMENTARIO_ID INT, U_ID INT) RETURNS INT
BEGIN
    DECLARE MIO INT DEFAULT 0;
    SET MIO = (SELECT COUNT(*) FROM COMENTARIO WHERE ID = COMENTARIO_ID AND USUARIO_ID = U_ID);
    RETURN MIO;
END __

CREATE FUNCTION ES_MI_LIKE(ELEMENTO_ID INT, U_ID INT, TIPO INT) RETURNS INT
BEGIN
    DECLARE MIO INT DEFAULT 0;
    IF TIPO = 1 THEN
        SET MIO = (SELECT COUNT(*) FROM CORAZON WHERE USUARIO_ID = U_ID AND PUBLICACION_ID = ELEMENTO_ID);
    ELSEIF TIPO = 2 THEN
        SET MIO = (SELECT COUNT(*) FROM CORAZON WHERE USUARIO_ID = U_ID AND COMENTARIO_ID = ELEMENTO_ID);
    END IF;
    RETURN MIO;
END __

CREATE FUNCTION CONTAR_LIKES(ELEMENTO_ID INT, TIPO INT) RETURNS INT
BEGIN
    DECLARE MIO INT DEFAULT 0;
    IF TIPO = 1 THEN
        SET MIO = (SELECT COUNT(*) FROM CORAZON WHERE PUBLICACION_ID = ELEMENTO_ID);
    ELSEIF TIPO = 2 THEN
        SET MIO = (SELECT COUNT(*) FROM CORAZON WHERE COMENTARIO_ID = ELEMENTO_ID);
    END IF;
    RETURN MIO;
END __

CREATE PROCEDURE ARMAR_PUBLICACION(IN PUBLICACION_ID INT, IN USUARIO_ID INT)
BEGIN
    SELECT P.ID, P.TEXTO_PUBLICACION, U.USERNAME, U.ID, P.F_PUBLICACION, P.TITULO, ES_PUBLICACION_MIA(PUBLICACION_ID, USUARIO_ID), LA_GUARDE_YO(PUBLICACION_ID, USUARIO_ID), ES_MI_LIKE(P.ID, USUARIO_ID, 1), CONTAR_LIKES(P.ID, 1), COUNT_CM(P.ID) FROM USUARIO U, PUBLICACION P WHERE P.ID = PUBLICACION_ID AND P.USUARIO_ID = U.ID;
END __

CREATE PROCEDURE ARMAR_COMENTARIOS(IN PUBLICACION_ID INT, IN USUARIO_ID INT)
BEGIN
    SELECT C.ID, C.USUARIO_ID, U.USERNAME, C.PUBLICACION_ID, C.F_PUBLICACION, C.TEXTO, ES_COMENTARIO_MIO(C.ID, USUARIO_ID), ES_MI_LIKE (C.ID, USUARIO_ID, 2), CONTAR_LIKES(C.ID, 2) FROM COMENTARIO C, USUARIO U WHERE C.USUARIO_ID = U.ID AND C.PUBLICACION_ID = PUBLICACION_ID;
END __

CREATE PROCEDURE CREACION_TAG(IN P_ID INT, IN T VARCHAR(45))
BEGIN
    DECLARE ENCONTRADO_2 INT DEFAULT 0;
    DECLARE ENCONTRADO INT DEFAULT 0;
    SET ENCONTRADO = (SELECT ID FROM TAG WHERE NOMBRE = T);
    IF ENCONTRADO IS NULL THEN
        INSERT INTO TAG (NOMBRE) VALUES (T);
        INSERT INTO TAG_PUBLICACION (PUBLICACION_ID, TAG_ID) VALUES (P_ID, LAST_INSERT_ID());
    ELSE
        SET ENCONTRADO_2 = (SELECT ID FROM TAG_PUBLICACION WHERE PUBLICACION_ID = P_ID AND TAG_ID = ENCONTRADO);
        IF ENCONTRADO_2 IS NULL THEN
            INSERT INTO TAG_PUBLICACION (PUBLICACION_ID, TAG_ID) VALUES (P_ID, ENCONTRADO);
        END IF;
    END IF;
END __
-- CALL CREACION_TAG(1, "NENENENEYHH") __ 
